...
function NetworkHelpLabels(ply)
	...
	for k, v in pairs(HelpCategories) do
		timer.Simple(n * .01, function() tNetworkHelpCategories(ply, v) end)
		n = n + 1
	end

	n = 0
	for k, v in pairs(HelpLabels) do
		timer.Simple(n * .01, function() tNetworkHelpLabels(ply, v) end)
		n = n + 1
	end
end
...
function GenerateChatCommandHelp()
	...
	AddHelpLabel(1350, HELP_CATEGORY_CHATCMD, p .. "g <Message> - Group message")
	...
	AddHelpLabel(2400, HELP_CATEGORY_CHATCMD, p .. "cr <Message> - Request the Combine")
end
